...
# from calendar import monthrange #double??
from cdo import Cdo
# import datetime #double??
import pytz
# import pickle #redundant through datetime??
import datetime
# import calendar
# import tempfile
# import struct
...
class Data(object):
    ...
    def _get_ny(self):
        if self.ndim == 2:
            ny, nx = self.content.shape
        elif self.ndim == 3:
            nt, ny, nx = self.content.shape
        else:
...
##        if notime:
##            if self.ndim == 2:
##                F.write(self._arr2string(self.content, prefix=''))
##            else:
##                raise ValueError(
...
##                F.write(self._arr2string(self.content, prefix='timmean'))
##            elif self.ndim == 3:
##                for i in xrange(len(self.time)):
##                    F.write(
##                        self._arr2string(self.content[i, :, :], prefix=str(self.date[i])))
##            else:
##                raise ValueError('Invalid geometry!')
##
##        F.close()
...
##
##        if hasattr(self, 'data'):
##            File.assign_value(varname, self.content)
##
##        if self.lat is not None:
...
##
##        if self.verbose:
##            print 'SQUEEZING data ... ', self.content.ndim, self.content.shape
##
...
##
##        if self.verbose:
##            print 'AFTER SQUEEZING data ... ', self.content.ndim, self.content.shape
    ...
    def _set_cell_area(self):
        ...
        if not self._calc_cell_area:
            if self.ndim == 2:
                self.cell_area = np.ones(self.content.shape)
            elif self.ndim == 3:
                self.cell_area = np.ones(self.content[0, :, :].shape)
            else:
                raise ValueError('Invalid geometry!')
            return
        ...
        if os.path.exists(cell_file):
            ...
            # check geometries
            if self.content.ndim == 2:
                if self.cell_area.shape != self.content.shape:
                    raise ValueError(
                        'Invalid cell_area file: delete it manually and check again!')
            elif self.content.ndim == 1:
                if self.cell_area.shape != self.content.shape:
                    raise ValueError(
                        'Invalid cell_area file: delete it manually and check again!')
            elif self.content.ndim == 3:
                if self.cell_area.shape != self.content[0, :, :].shape:
                    raise ValueError(
                        'Invalid cell_area file: delete it manually and check again!')
        else:
            ...
            if self.content.ndim == 2:
                ...
                print 'actual geometry:  ', self.content.ndim, self.content.shape
                raise ValueError('Invalid geometry!')
...
##        return res
    ...
    def get_percentile(self, p, return_object=True):
        ...
        # reshape data array
        res.shape = np.shape(self.content[0, :, :])
...
##
##        if len(mask) != len(self.content):
##            print len(mask), self.content.shape
##            raise ValueError('Inconsistent length of data and mask')
##
##        for i in xrange(len(mask)):
##            if mask[i]:
##                self.content.mask[i, :, :] = True
    ...
    def read(self, shift_lon, start_time=None, stop_time=None,
        ...
        np.ma.masked_where(np.isnan(self.content), self.content, copy=False)

        # this scaling is related to unit conversion and NOT
        # due to data compression
        self.content *= self.scale_factor
...
##            return res
    ...
    #~ def correlate(self, Y, pthres=1.01, spearman=False, detrend=False):
        ...
        #~ """
#~
        #~ if not Y.data.shape == self.content.shape:
            #~ print Y.data.shape, self.content.shape
            #~ raise ValueError('unequal shapes: correlation not possible!')
#~
        #~ # generate a mask of all samples that are valid in BOTH datasets
        #~ vmask = self.content.mask | Y.data.mask
        #~ vmask = ~vmask
#~
        #~ # copy original data
        #~ xv = self.content.copy()
        #~ yv = Y.data.copy()
        #~ sdim = self.content.shape
#~
...
##        if self.content.ndim > 1:
##            clim = np.ones(np.shape(self.content[0:self.time_cycle, :])) * np.nan
##            slim = np.ones(np.shape(self.content[0:self.time_cycle, :])) * np.nan
##        else:
##            clim = np.ones(np.shape(self.content[0:self.time_cycle])) * np.nan
##            slim = np.ones(np.shape(self.content[0:self.time_cycle])) * np.nan
##
...
##                clim[i::self.time_cycle] = self.content[
##                    i::self.time_cycle].mean(axis=0)
##                slim[i::self.time_cycle] = self.content[
...
##        ret = np.ones_like(self.content) * np.nan
##
##        if ret.ndim == 1:
##            for i in xrange(self.time_cycle):
##                ret[i::self.time_cycle] = self.content[i::self.time_cycle] - \
##                    clim[i]
##        elif ret.ndim == 2:
##            for i in xrange(self.time_cycle):
##                ret[i::self.time_cycle, :] = self.content[
##                    i::self.time_cycle, :] - clim[i, :]
##        elif ret.ndim == 3:
##            for i in xrange(self.time_cycle):
##                ret[i::self.time_cycle, :, :] = self.content[i::self.time_cycle, :, :] - clim[i, :, :]
##        else:
##            raise ValueError('Invalid dimension when calculating anomalies')
##        ret = np.ma.array(ret, mask=(np.isnan(ret) | self.content.mask))
##
...
##
##        if self.content.ndim == 2:
##            if self.content.shape != m.shape:
##                print self.shape
##                print m.shape
##                raise ValueError('Invalid geometry!')
##        elif self.content.ndim == 3:
##            if self.content[0, :, :].shape != m.shape:
##                print self.shape
##                print m.shape
##                raise ValueError('Invalid geometry!')
##        else:
...
##                                                          i], maxs[0, i] = _get_stat(self.content, m, vals[i])
##
##        elif self.content.ndim == 3:
##            nt = len(self.content)
...
##
    ...
    def _temporal_subsetting(self, i1, i2):
        ...
        if self.content.ndim == 3:
            self.content = self.content[i1:i2, :, :]
        elif self.content.ndim == 2:
            ...
            if self.squeezed:
                ...
                self.content = self.content[i1:i2, :]
        elif self.content.ndim == 1:  # single temporal vector assumed
            self.content = self.content[i1:i2]
        else:
...
##        return res
    ...
    def timmean(self, return_object=False):
        ...
        """
        if self.content.ndim == 3:
            res = self.content.mean(axis=0)
        elif self.content.ndim == 2:
            res = self.content.copy()  # no temporal averaging
        else:
            print self.content.ndim
            raise ValueError(
...
##        dmin = self.content.min()
##        x = self.content.flatten() - dmin + 1.
...
##            return y
    ...
    def _get_weighting_matrix(self):
        ...
        if self.content.ndim == 2:
            ...
            w.shape = self.content.shape  # geometry is the same now as data

            if normtype == 'valid':
                # 2) mask areas that do not contain valid data
                w = np.ma.array(w, mask=self.content.mask)
...
##            # no area weighting
##            if self.content.ndim == 3:
##                tmp = np.reshape(self.content, (len(self.content), -1)).sum(axis=1)
##            elif self.content.ndim == 2:
##                tmp = np.asarray([self.content.sum()])
##            else:
##                raise ValueError('Undefined')
##
...
##            return tmp
    ...
    def fldmean(self, return_data=False, apply_weights=True):
        ...
        if apply_weights:
            ...
            # no area weighting
            if self.content.ndim == 3:
                tmp = np.reshape(self.content, (len(self.content), -1)).mean(axis=1)
            elif self.content.ndim == 2:
                tmp = np.asarray([self.content.mean()])
            else:
                raise ValueError('Undefined')
    ...
    def fldstd(self, return_data=False, apply_weights=True, ddof=0):
        ...
        * unittest together with cdo's
        ...
        if apply_weights:
            ...
            if self.content.ndim == 2:
                mu = (self.content * w).sum() / w.sum()
                if ddof == 0:  # unbiased estimator
                    V1 = w.sum()
                    s = (w * (self.content - mu) ** 2.).sum() / V1
                    tmp = [np.sqrt(s)]
                    ...
                    print 'V1, V2: ', V1, V2
                    print 'mu: ', mu
                    s = V1 * np.sum(w * (self.content - mu) ** 2.) / \
                        (V1 * V1 - V2)
                    tmp = [np.sqrt(s)]
                else:
                ...
                if ddof == 0:
                    for i in xrange(nt):
                        V1 = w[i, :, :].sum()
                        mu = (self.content[i, :, :] * w[i, :, :]).sum() / V1
                        s[i] = (w[i, :, :] * (self.content[i, :, :] - mu)
                                ** 2.).sum() / V1
                    tmp = np.sqrt(s)
                elif ddof == 1:
                    for i in xrange(nt):
            ...
            # no area weighting
            if self.content.ndim == 2:
                tmp = self.content.std(ddof=ddof)
            elif self.content.ndim == 3:
                tmp = np.reshape(self.content, (len(self.content), -1)
...
##
##        if self.content.ndim == 1:
##            thefrac = 1. - float(sum(self.content.mask)) / \
##                float(len(self.content.mask))  # valid fraction
##            if thefrac >= frac:
...
##            res = np.ones(self.content.shape).astype('bool')
##            res[self.content.mask] = False
##            if return_frac:
##                thefrac = np.ones(self.content.shape)
##                return res, thefrac
##            else:
##                return res
##        elif self.content.ndim == 3:
##            n = len(self.content)  # number of timesteps
##            hlp = self.content.copy()
##            if hasattr(hlp, 'mask'):
##                hlp1 = hlp.data.copy()
...
##            msk = msk1
##
##        self.__oldmask = self.content.mask.copy()
##        self.__olddata = self.content.data.copy()
##        if hasattr(self, 'std'):
##            if self.content.shape != self.std.shape:
##                raise ValueError(
...
##            tmp1 = self.content.copy().astype('float')
##            tmp1[~msk] = np.nan
##
##            if hasattr(self, 'std'):
...
##
##            self.content = np.ma.array(tmp1, mask=np.isnan(tmp1))
##            if hasattr(self, 'std'):
...
##                tmp = self.content[i, :, :].copy()
##                tmp[~msk] = np.nan
##                self.content[i, :, :] = tmp[:, :] * 1.
##                del tmp
##
##                if hasattr(self, 'std'):
...
##            if keep_mask:
##                if self.__oldmask.ndim > 0:
##                    self.content.data[self.__oldmask] = np.nan
##                    if hasattr(self, 'std'):
##                        self.std.data[self.__oldmask] = np.nan
##
##            self.content = np.ma.array(
##                self.content.data, mask=np.isnan(self.content.data))
##            if hasattr(self, 'std'):
##                self.std = np.ma.array(
##                    self.std.data, mask=np.isnan(self.std.data))
##        else:
##            print np.shape(self.content)
...
##        """
##        if self.content.ndim == 3:
##            self.content = self.__shift3D(self.content, nx)
##        else:
##            self.content = self.__shift2D(self.content, nx)
...
##        """
##        self.content = np.ma.array(
##            self.content, mask=((self.content < vmin) | (self.content > vmax)))
##
...
##        """
##
##        if np.shape(self.content) != np.shape(x.data):
...
##
    ...
    def sub(self, x, copy=True):
        ...
        f_elementwise = False
        if np.shape(self.content) != np.shape(x.content):
            s1 = np.shape(self.content)
            if (s1[1] == x.content.shape[0]) & (s1[2] == x.content.shape[1]):
                f_elementwise = True
            else:
        ...
        if f_elementwise:
            for i in xrange(len(d.content)):
                d.content[i, :, :] = d.content[i, :, :] - x.content[:, :]
        else:
            d.content = d.content - x.content
        d.label = self.label + ' - ' + x.label
        return d
...
##        if axis > len(self.content.shape) - 1:
...
##            the same geometry as the second and third dimension of self.content)
##        copy : bool
...
##            geometry as self.content or second and third dimension need to match)
##        copy : bool
##            if True, then a new data object is returned
##            else, the data of the present object is changed
##        Test
...
##            if self.content.ndim == 3:
##                if x.data.ndim == 2:
##                    if np.shape(self.content[0, :, :]) == np.shape(x.data):
...
##            # slow implementation ...
##            for i in xrange(self.nt):
##                o.data[i, :, :] = self.content[i, :, :] * x[i]
##        else:
##            # fast implementation using broadcasting ...
##            # http://docs.scipy.org/doc/numpy/user/basics.broadcasting.html
##            o.data[:, :, :] = self.content[:, :, :] * x[:, np.newaxis, np.newaxis]
##
##        if copy:
##            return o
##        else:
...
##        """
##        if self.content.ndim == 3:
##            self.content = self.content[:, ::step, ::step]
##        elif self.content.ndim == 2:
##            self.content = self.content[::step, ::step]
##        else:
...
##        reg = Data(None, None)
##        reg.data = np.zeros(self.content.shape) * np.nan
##        reg.label = 'trend line'
##        nt = self.content.shape[0]
##        for i in range(nt):
...
##
##        if self.content.ndim == 1:  # single timeseries
##            tmp = np.ones_like(self.content) * np.nan
##            r = _runningMeanFast(self.content.flatten(), N)
##            tmp[N / 2:len(tmp) - N / 2] = r[:-(N - 1)]  # center data!
##        elif self.content.ndim == 2:
##            raise ValueError(
##                'Invalid data geometry for temporal smoothing! (2D)')
##        elif self.content.ndim == 3:
##            r = np.ones_like(self.content) * np.nan
##            nt, ny, nx = self.content.shape
##            for i in xrange(ny):  # todo: more efficient implementation
##                for j in xrange(nx):
##                    if msk[i, j]:
##                        r[:, i, j] = _runningMeanFast(self.content[:, i, j], N)
...
##
##        self.content = np.ma.array(data, mask=data != data)
##        self.verbose = True
##        self.unit = 'myunit'
...
##                            t, P, self.content[:, i, j], corr=corr)
##                    else:
