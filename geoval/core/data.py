...
class GeoData(object):
    ...
    def partial_correlation(self, Y, Z, ZY=None, pthres=1.01, return_object=True):
        ...
        assert isinstance(Y, GeoData)
        assert isinstance(Z, GeoData)

        # if a second condition is given, use it ...
        if ZY is not None:
            assert isinstance(ZY, GeoData)
        else:
            ZY = Z
    ...
    def _copy_all_attributes(self, d):
        """
        copy all attributes to new object
    ...
    def _is_daily(self):
        ...
        """
        if hasattr(self, 'time'):
            d = np.ceil(plt.date2num(self.date))
    ...
    def mask_region(self, r, return_object=True, method='full', maskfile=None, force=False):
        ...
        if f_rasterize:
            ...
            # load mask from file!
            MD = GeoData(maskfile, 'mask', read=True)
    ...
    def get_temporal_mask(self, v, mtype='monthly'):
        ...
        # get months or years
        if mtype == 'monthly':
            vals = np.array(self._get_months())
        elif mtype == 'yearly':
            vals = np.array(self._get_years())
        else:
            raise ValueError('Invalid type for mask generation %s ' % mtype)
